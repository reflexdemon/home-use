...
public class OAuthLoginHandler extends AuditService
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        try {
            String provider = (String) userAttrMap.get(Constants.PROVIDER_ID);
            User currentUser = buildUserFromOAuth(userAttrMap);
            User user = getUserFromDB(currentUser);
            //Creates a new HTTP Session
            request.getSession(true).setAttribute(Constants.LOGGED_IN_USER, user);
        } catch (Exception e) {
            logger.error("Unable to get the user", e);
        }

        Mono<OAuth2Log> logMono = saveBlockingLog(request, authentication, userAttrMap, Constants.LOGIN_SUCCESS);
    ...
    private String getRedirectURL(HttpServletRequest request) {
        ...
        String requestURL = "/";
        Object savedRequest = session.getAttribute(Constants.SPRING_SECURITY_SAVED_REQUEST);
